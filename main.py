...
def block():
    ...
    if websites_to_block:
        ...
        Label(root, text="Blocked", font="arial 12 bold").place(x=210, y=300)
    else:
        Label(root, text="All websites already blocked", font="arial 12 bold").place(x=150, y=300)
...
def unblock():
    ...
    Label(root, text="Unblocked", font="arial 12 bold").place(x=210, y=300)
...
Label(root, text='Select Your OS:', font=("arial", 12, "bold")).place(x=60, y=130)

# OS dropdown menu with values
os_dropdown = ttk.Combobox(root, values=["Windows", "Mac", "Linux"], state="readonly", width=10)
os_dropdown.place(x=187, y=125)
os_dropdown.current(0)  # Set default to Windows

# Button to confirm OS selection
os_button = Button(root, text='CONFIRM OS', font='arial 12 bold', command=set_os_path, width=10, bg='hot pink', activebackground= 'sky blue', fg='white')
os_button.place(x=325, y=125)

# Label prompting user input
Label(root, text='Enter Website:', font = ("arial", 12, "bold")).pack(anchor= "center")

# Websites to block entry port
...
block_button.place(x= 140, y=215)
...
unblock_button.place(x=270, y=215)
